...
opeb.raw <- opeb.raw %>%
  mutate(AAL_standard =  (AAL_reported*((1+disc_rate)^15) )/ (1.04)^15)
...
pension.raw <- pension.raw %>% 
            mutate(TotalPensionLiability = case_when(is.na(TotalPensionLiability) ~ 
                                     (NetLiability + Assets),
...
The unfunded obligations of the pension and Other Post-Employment Benefits (OPEB) plans sponsored by local governments in the United States continue to grow. In the following report, we study in detail the financial obligations the 20 largest US cities (by population). We report on both their own measurements of their obligations, and how these differ from valuations using more realistic assumptions.

We find that most cities reported OPEB liabilities close to our measures, however, for pensions, we find that cities drastically undervalue their unfunded liabilities. In fact, at the end of the 2017 fiscal year, the 20 largest U.S cities reported unfunded liabilities of over **\$253.7**  billion: **\$140.6** billion for pensions and **$113.1** billion for OPEB. According to our calculations, we estimate the true unfunded liabilities to be **\$1.97** trillion: **\$1863.8** billion for pensions and **\$111.3** billion for OPEB.
The discrepancy between our valuations and the cities' reports results from the fact that the selection of the discount rate as per the government accounting standards practices is at the discretion of a city, rather than being standardized by a federal authority, hence allowing cities to measure liabilities by choosing their own expected return on plan assets.
In order to place cities on a more comparable footing, we decided to use the **15 year AA corporate bond rate** (June 2017), approximately **4.00%**, as the discount rate for both pension and OPEB for all plans sponsored by the 20 cities in our valuations as FASB stipulates that local governments must use a high-quality municipal bond rate as the discount rate. We will further discuss the impact of the discount rate on liability calculations below.  
...
As FASB stipulates that local governments must use a high-quality municipal bond rate as the discount rate,
we decided to use the 15 year AA corporate bond rate (June 2017), approximately **4.00%**, as the discount rate for both pension and OPEB for all cities in our valuations in order to place cities on a more comparable footing. It is however important to note that any value for the discount rate still remains an assumption on the expected returns of the assets of city plans.  
...
The rest of this report is organized as follows. First, we present in detail the pension liability of the 20 cities. Second, we investigate their OPEB liability. Finally, we combine the cities' pension and OPEB liabilities to present a full snapshot of their financial obligations. 
...
The difference in the reported pension liabilty and the ones calculated under our standardized valuation are illustrated in Figure 2 below. It appears that cities report much lower unfunded liabilities. For almost all cities, the standardized pension liability is greater than the reported value. Fort Worth, Philadelphia, San Diego and San Jose are the only cities that reported an unfunded pension liability lower than our standardized measure. Once again, we can see the sensitivity of the unfunded liabilities to the discount rate. 
...
### 2. Total Pension Per Capita

The difference in the reported pension liability and our own valuations scaled by population size are illustrated in Figure 3 below. Similar to OPEB, we provide per capita measures to present city residents with a better idea of their city's pension financial burden and how it compares with that of other cities. The average reported unfunded pension liability per capita was \$`r round(mean(total.summary$Pen_reported.capita,na.rm=TRUE),1)*1000` while the average standardized unfunded pension liability per capita was \$42,100.
...
The difference in the reported UAALs and the ones calculated under our standardized valuation are illustrated in Figure 4 below. It appears that cities report lower unfunded liabilities. For almost all cities, the standardized UAAL is greater than the reported value. Only Chicago, Indianapolis, New York and Seattle reported a UAAL lower than our standardized measure. This makes sense, because as we explained earlier, the higher the discount rate, the lower the unfunded liability and as illustrated in Figure 1 most cities use a discount rate higher than our standardized one.
...
The difference in the reported total liabilty, i.e pension and OPEB combined, and the ones calculated under our standardized valuation are illustrated in Figure 7 below. In accordance with our results from the previous two sections, we find that cities tend to report lower unfunded liabilities. For almost all cities, the standardized total liability is greater than the reported value. The average reported total liability was a little less than **$13 billion** while the average standardized total liability was a little over **$100 billion**. Only three cities out of the 20: Philadelphia, San Diego and San Jose reported an unfunded total liability lower than our standardized measure. Once again, we can see the sensitivity of the unfunded liabilities to the discount rate. 
...
Similar to OPEB and pension, we also provide in Figure 8 below the total liability per capita for all the cities included in our report. As one would expect, the total unfunded liabilities these cities reported were usually lower than our standardized measures. The average reported total liability per capita was **$4,400** while the average standardized total liability per capita was **$43,500**. 
